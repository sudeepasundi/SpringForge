...
import { QuestionCard } from '@/components/java/QuestionCard';
...
import {
  ...
  questionTopics,
  type JavaQuestion,
} from '@/content/java/questions';
import { cn } from '@/lib/cn';

const topicLabel = new Map<string, string>(questionTopics.map((t) => [t.id, t.label]));
const chapterTitle = new Map(javaBook.chapters.map((c) => [c.slug, c.title]));

function matches(q: JavaQuestion, text: string): boolean {
  if (!text) return true;
...
export default function JavaRevisionPage() {
  ...
  return (
    <div className="mx-auto max-w-[62rem] px-5 py-10 sm:px-8">
      ...
      >
        <Link to="/java" className="hover:text-[color:var(--sf-text)]">
          Java
        </Link>
        <span>/</span>
      ...
      <header className="mb-6">
        ...
        <p className="mt-2.5 mb-0 max-w-[64ch] text-[0.98rem] leading-relaxed text-[color:var(--sf-text-muted)]">
          {javaQuestions.length} questions that come up in Java interviews, with the answer you
          would give, the follow-ups to expect, and a link to the chapter that explains it
          properly. Try answering before you open a card.
        </p>
      </header>
      ...
      <div className="sticky top-[var(--sf-header-h)] z-10 -mx-2 mb-5 bg-[color:var(--sf-bg)]/90 px-2 py-3 backdrop-blur-md">
        ...
        <div className="mt-2.5 flex items-center justify-between text-[0.76rem] text-[color:var(--sf-text-faint)]">
          <span aria-live="polite">
            {visible.length} of {javaQuestions.length} shown
          </span>
          {visible.length > 0 && (
            <button
              type="button"
